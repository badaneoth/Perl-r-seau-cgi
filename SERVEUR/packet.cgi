...
#my $install=$file;
#my $install=$ARGV[0];
my $install=$ajax->getDataFromClient();
...
my $sock = new IO::Socket::INET(
	...
	PeerPort => '5123', # Port the udp server listens on
	Proto => 'udp',
...
# Etape 5
	if(scalar @diffRep > 0){
		...
		close (SUPP);
	#print  "-----------------------------------------------------------------------\n";
	$ajax->sendResultToClient("Le packet $install est prêt à être déployé !");
...
sub copier_dif_repertoires{
	...
	foreach my $chemin (@diff){
		...
		if(-d $chemin){
			...
			my $monDir= mon_dir($chemin);
			my $dir=nom_logiciel($monDir);
			my $vers=$dossier1;
