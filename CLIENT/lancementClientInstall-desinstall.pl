...
sub copier{
	...
	$install=$install.".txt";
	print "$install\n";
	open(my $fd,"<$install") or die"open: $!";
		my($line);
...
sub desinstalleCopier{
	...
	open(my $fd,"<$install") or die"open: $!";
		my($line);
		while( defined( $line = <$fd> ) ) {	
			$cheminVers=$line."/".$rec[0];	
		}
	close($fd);
	if(rmtree([$cheminVers], 1, 1)!=0)
	{
		print "1Suppression de fichier $cheminVers\n";
	}
	if(rmtree([$install], 1, 1)!=0)
	{
		print "2Suppression de fichier $install\n";
	}
	if(rmtree([$dossier], 1, 1)!=0)
	{
		print "3Suppression de fichier $dossier\n";
	}
